...
const Directorate = ({ activeTab }) => {
  ...
  useEffect(() => {
    ...
    if (activeTab === "wc_registrations") {
      ...
      axiosInstance.get(`${API}/api/admin/wellness-centre-operational-registrations`)
        .then(res => { if (res.data.success) setWcRegs(res.data.data); })
        .catch(err => console.error('Error fetching WC regs:', err))
        .finally(() => setWcRegsLoading(false));
    }
    ...
    return (
      <div style={{ padding: 24, minHeight: '100vh', background: '#f8fafc' }}>
        <div style={{ marginBottom: 24 }}>
          ...
          <p style={{ color: '#64748b', marginTop: 4 }}>View operational registration applications from Wellness Centre users across all districts.</p>
        </div>
        ...
        {wcRegsLoading ? (
          ...
          <div style={{ textAlign: 'center', padding: '60px', background: '#fff', borderRadius: 12, border: '1px solid #e2e8f0', color: '#64748b' }}>
            No wellness centre registration applications found.
          </div>
        ) : (
