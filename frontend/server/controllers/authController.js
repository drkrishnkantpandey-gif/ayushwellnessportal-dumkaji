...
const loginUser = async (req, res) => {
  ...
  try {
    ...
    const globalUserCheck = await client.query(
      'SELECT id, email, password_hash, role, is_verified, full_name FROM users WHERE LOWER(TRIM(email)) = $1',
      [normalizedEmail]
    );
    ...
    if (normalizedRole === 'ayush_college') {
      ...
      user = { ...dbUser, role: dbRole }; // Ensure role is lowercase consistent with selection
    }
